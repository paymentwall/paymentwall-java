Feature: Widget
  ...
  Scenario: check Cart payment widget signature v2 with correct secret key
    ...
    When Widget is constructed
    And Widget HTML content is loaded
    Then Widget URL should contain "/api/cart?"
    And Widget HTML content should not contain "It looks like you're not authorized to view this content."
    And Widget HTML content should contain "Total:"

  Scenario: check Cart payment widget signature v2 with wrong secret key
    Given Public key "cbb80a5cd091231ac073a06b1d1c83ff"
    And Secret key "51909b7b8b333e0f9de3c2cwronga3c"
    And API type "3"
    And Widget signature version "2"
    And Product id "1й"
    And Widget code "p1"
